...
import static com.example.canister.med_manager.database.MedicineContract.MedicineEntry.MEDDICINE_INTAKE_TABLE_NAME;
...
/**
 * Provides data for user intake
 * Used for tracking medication intake purposes
 * **/

public class MedicineIntakeContentProvider extends ContentProvider {


    //Defining constants for all data and single data item
    public static final int MEDICINES_INTAKE = 100;
    public static final int MEDICINES_INTAKE_WITH_ID = 101;
    ...
    public static UriMatcher buildUriMatcher(){
        ...
        //match all data
        uriMatcher.addURI(MedicineContract.AUTHORITY, MedicineContract.PATH_MEDICINE_INTAKE, MEDICINES_INTAKE);
        //match single item
        uriMatcher.addURI(MedicineContract.AUTHORITY, MedicineContract.PATH_MEDICINE + "#", MEDICINES_INTAKE_WITH_ID);
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection, @Nullable String[] selectionArgs, @Nullable String sortOrder) {
        ...
        switch (match) {
            //all data
            case MEDICINES_INTAKE:
                cursor =  mDb.query(MEDDICINE_INTAKE_TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
                        ...
                        sortOrder);
                break;
            case MEDICINES_INTAKE_WITH_ID:
                selection = MedicineEntry._ID + "=?";
                selectionArgs = new String[] { String.valueOf(ContentUris.parseId(uri)) };

                cursor = mDb.query(MedicineEntry.MEDDICINE_INTAKE_TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
            default:
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues contentValues) {
        ...
        switch (match) {
            case MEDICINES_INTAKE:
                ...
                long newRowId = mDb.insert(MEDDICINE_INTAKE_TABLE_NAME, null, contentValues);

                if ( newRowId == -1) {
                    throw new android.database.SQLException("Failed to insert row into " + uri);
                } else {
                    returnUri = ContentUris.withAppendedId(MedicineEntry.CONTENT_URI, newRowId);
                }
                break;
            default:
    ...
    public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case MEDICINES_INTAKE_WITH_ID:
                ...
                medicineDeleted = db.delete(MEDDICINE_INTAKE_TABLE_NAME, "_id=?", new String[]{id});
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
